        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <div className={`transition-all duration-1000 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            ...
            <h2 className="text-3xl lg:text-4xl xl:text-5xl font-bold text-espresso leading-tight mb-6" style={{ fontFamily: "var(--font-display)" }}>
              Mentorship &amp;<br /><span className="text-ember italic">Discipleship</span>
            </h2>
            ...
            <blockquote className="border-l-2 border-amber pl-5 mb-10">
              <p className="font-serif italic text-umber/70 text-base leading-relaxed" style={{ fontFamily: "var(--font-serif)" }}>
                &ldquo;The things that thou hast heard of me&hellip; commit thou to faithful men.&rdquo;
              </p>
          ...
          {/* Right — image-backed pillar cards */}
          <div className={`transition-all duration-1000 delay-200 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>

            {/* Large image card on top */}
            <div className="relative overflow-hidden mb-3" style={{ aspectRatio: "16/7" }}>
              <Image src="/images/pic2.jpeg" alt="Mentorship" fill className="object-cover object-[center_30%]" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to right, rgba(42,27,18,0.75) 0%, rgba(42,27,18,0.20) 60%, transparent)" }} />
              <div className="absolute inset-0 flex items-center px-8">
                <div>
                  <p className="text-[10px] tracking-[0.2em] uppercase text-amber font-sans font-semibold mb-2">One-on-one · Group · Online</p>
                  <p className="text-parchment font-bold text-xl leading-snug" style={{ fontFamily: "var(--font-display)" }}>
                    Structured.<br />Spirit-led. Personal.
                  </p>
                </div>
              </div>
            </div>

            {/* 2x2 pillar grid below */}
            <div className="grid grid-cols-2 gap-px bg-sand/40">
              {PILLARS.map((p, i) => (
                <div key={i} className="bg-white p-6 hover:bg-espresso group transition-colors duration-300">
                  <span className="text-xl text-amber block mb-3 group-hover:scale-110 transition-transform duration-300">{p.icon}</span>
                  <h4 className="font-bold text-espresso group-hover:text-parchment text-base mb-1 transition-colors" style={{ fontFamily: "var(--font-display)" }}>{p.title}</h4>
                  <p className="text-xs font-sans text-umber/60 group-hover:text-parchment/50 leading-relaxed transition-colors">{p.desc}</p>
                </div>
              ))}
            </div>
          </div>
